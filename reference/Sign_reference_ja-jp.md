...
[　: （define　中置演算子　※）	14](#　:-（define　中置演算子　※）)

[　\# （output export　前置演算子）	14](#　#-（output-export　前置演算子）)

[　  （余積　中置演算子）	15](#　-（余積　中置演算子）)

[　? （ラムダ構築　中置演算子　※）	16](#　?-（ラムダ構築　中置演算子　※）)

[　, （積　中置演算子　※）	18](#　,-（積　中置演算子　※）)

[　\~ （範囲リスト構築　中置演算子）	18](#　~-（範囲リスト構築　中置演算子）)

[　\~　（残余引数リスト構築　前置演算子）	19](#　~　（残余引数リスト構築　前置演算子）)

[　;　（xor　中置演算子）	20](#　;　（xor　中置演算子）)

[　|　（ or　中置演算子）	20](#　|　（-or　中置演算子）)

[　&　（and　中置演算子）	20](#　&　（and　中置演算子）)

[　\!　（not　前置演算子）	20](#　!　（not　前置演算子）)

[　\< （less　中置演算子）	21](#　\<-（less　中置演算子）)

[　\<= （less equal　中置演算子）	21](#　\<=-（less-equal　中置演算子）)

[　= （equal　中置演算子）	21](#　=-（equal　中置演算子）)

[　\>= （more equal　中置演算子）	21](#　\>=-（more-equal　中置演算子）)

[　\> （more　中置演算子）	21](#　\>-（more　中置演算子）)

[　\!=　（not equal　中置演算子）	21](#　!=　（not-equal　中置演算子）)

[　+ （加法　中置演算子）	22](#　+-（加法　中置演算子）)

[　- （減法　中置演算子）	22](#　--（減法　中置演算子）)

[　\* （乗法　中置演算子）	22](#　*-（乗法　中置演算子）)

[　/ （除法　中置演算子）	22](#　/-（除法　中置演算子）)

[　% （剰余　中置演算子）	22](#　%-（剰余　中置演算子）)

[　^ （冪乗　中置演算子　※）	22](#　^-（冪乗　中置演算子　※）)

[　\! （階乗　後置演算子）	22](#　!-（階乗　後置演算子）)

[　\~ （展開　後置演算子）	23](#　~-（展開　後置演算子）)

[　’ （get　中置演算子）	24](#　’-（get　中置演算子）)

　[（評価　後置演算子）	24](#（評価　後置演算子）)

[　@　（input import　前置演算子）	25](#　@　（input-import　前置演算子）)

[　ブロック構築について　25](#　ブロック構築について)
...
* 0と空のリストはfalseであり、何らかの存在が確認される場合はtrueであるため、ブーリアンという型が明示されることはない  
* 論理演算は全て短絡評価である  
...
## 　\#	（output export　前置演算子） []({#　#-（output-export　前置演算子）})

具体的なIOアドレスやメモリアドレスを取ることも出来る。  
型としては以下の通り。
...
（例２　再帰関数でprint関数をシステムコールのためのアドレスを用いて定義）  
\#print : s \~t ?  
	\#0x40 : s  
	@print t\~
...
* 引数リストに渡す引数の羅列について、明示する必要がある場合 , 中置演算子を用いる
...
2. ### ポイントフリースタイルによる記述    この方法は、演算子を直接関数として扱う方法を提供するが、関数適用を挟むことになるため、演算子の優先順位がなくなることに注意！    型は以下の通り
   ...
   * 後置演算子のポイントフリー化は、演算子の前に\_をつけて表現する  
     （例）  
     \[\!\] 5 \= \!5  
     \[\_\!\] 5 \= 5\!
...
3. ### ラムダはリストに対する自然変換を提供する    この方法は、積演算子である , を、カッコの中のラムダ式でテキスト置換するという大まかなイメージを考えると成立する 
...
\~ 中置演算子は、範囲指定など、範囲を抽象的に取り扱うことが出来る。  
型は以下の通り。
...
## 　\~　（残余引数リスト構築　前置演算子） []({#　~　（残余引数リスト構築　前置演算子）})
...
Signでは、数値の 0 または 空のリストがfalseとなっているため、明示的なブーリアンが無い。  
この演算子は、あくまで論理演算である。  
論理演算は、全て短絡評価となる。
...
【or リスト or 文字列】 ’ 【識別子 or 文字列 or 数値】  
...
## 　@　（input import　前置演算子） []({#　@　（input-import　前置演算子）})

@ 前置演算子は、ファイルやライブラリをインポートしたり、IOアドレスからデータを取得する際に使われる。型は以下の通り。
