...
const tokenizeAffixes = code => code.replace(
  /((?:[#~!$@]|!!)([^\s]+))|(([^\s]+)([!~@]))|(\\[\s\S])|(`[^`\r\n]*`)/g,
  (m, p1, p2, p3, p4, p5, esc, lit) => {
    if (esc || lit) return esc || lit;
    if (p1) { // prefix: op + operand
      const op = p1.match(/^([#~!$@]|!!)/)[0];
      const rest = p2;
      return `${op}_ ${rest.match(/^(?:[#~!$@]|!!)[^\s]+/) ? tokenizeAffixes(rest) : rest}`;
    }
    if (p3) { // postfix: operand + op
      const operand = p4;
      const op = p5;
      return `${operand.match(/[^\s]+(?:[!~@])$/) ? tokenizeAffixes(operand) : operand} _${op}`;
    }
    return m;
  }
);

const preprocess = code => tokenizeAffixes(
  code
