...
// currentChar
logger.progress('currentChar', this.position , this.currentChar);
...
if(cnt < tokens.length){
    logger.progress(`追加されたトークン:` , `${tokens.length-1}`, `${JSON.stringify(tokens[tokens.length-1])}`);
    cnt++;
}
            ...
            // デバッグ用にトークン数を出力
            logger.debug(`字句解析完了: ${tokens.length} 個のトークンを生成`);
            ...
            logger.error(`字句解析エラー (${this.line}行目, ${this.column}列): ${error.message}`);
            throw error;
        }
    }
...
/**
    ...
    readString() {
        ...
        while (this.currentChar !== null && this.currentChar !== '`' && this.currentChar !== '\n') {
            ...
            result += this.currentChar;
// currentChar
logger.progress('currentChar_readString', this.position , this.currentChar);
