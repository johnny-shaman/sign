...
namespace sign
{
    ...
    /**
     * Sign言語コードをファイルに出力する (common::writeToFileへの転送)
     *
     * @param code 出力するコード
     * @param filename 出力ファイル名
     * @return 成功した場合はtrue
     */
    inline bool writeToFile(const std::string &code, const std::string &filename)
    {
        return common::writeToFile(code, filename);
    }
